...
{
	public class Replica_Sieru_flail_Projectile : ModProjectile
	{
		public override string Texture
		{
			get { return "KsonFriends/Items/Replica_Sieru_flail/Replica_Sieru_flail_Projectile"; }
		}

		// This file shows an animated projectile
		// This file also shows advanced drawing to center the drawn projectile correctly
		private const string ChainTexturePath = "KsonFriends/Items/Replica_Sieru_flail/Replica_Sieru_flail_Projectile_chain"; // The folder path to the flail chain sprite
		private const string ChainTextureExtraPath = "KsonFriends/Items/Replica_Sieru_flail/Replica_Sieru_flail_Projectile_chain_extra" +
            "";  // This texture and related code is optional and used for a unique effect
		...
		public override void AI()
		{
			...
			float maxRetractSpeed = 14f; // The max speed the projectile will have while retracting
				...
				case AIState.Spinning:
					{
						...
						Vector2 offsetFromPlayer = new Vector2(player.direction).RotatedBy((float)Math.PI * 5f * (SpinningStateTimer / 60f) * player.direction);
						...
						Projectile.Center = mountedCenter + offsetFromPlayer * 35f;
			...
			*/
			...
			if (Main.rand.NextBool(dustRate))
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.MinecartSpark, 0f, 0f, 150, default(Color), 1.3f);
		}
